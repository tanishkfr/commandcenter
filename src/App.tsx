...
export default function App(){
  ...
  const load=async(projectId?:string,sessionId?:string)=>{setLoading(true);setFatalError('');try{const next=await studioApi.bootstrap(projectId);setData(next);if(sessionId)setSession(await studioApi.getSession(sessionId));else setSession(next.activeSession)}catch(error:any){const message=error?.message||'Could not open the studio.';setFatalError(message);showToast(message)}finally{setLoading(false)}};
  useEffect(()=>{load()},[]);
  ...
  if(!data)return <main className="deployment-repair"><section><Mark/><p className="eyebrow">Deployment setup</p><h1>Studio needs its cloud memory.</h1><p>{fatalError||'The application API is not ready yet.'}</p><ol><li>Connect a <strong>Private Vercel Blob</strong> store to this project.</li><li>Redeploy the latest commit so the API function receives its storage credentials.</li><li>Open <code>/api/health</code> and confirm that it returns JSON.</li></ol><div><button onClick={()=>load()}><RotateCcw size={14}/>Try again</button><a href="https://vercel.com/dashboard" target="_blank" rel="noreferrer">Open Vercel dashboard<ArrowUpRight size={13}/></a></div><small>Local development still uses .memory/studio.json and does not require Blob.</small></section></main>;
