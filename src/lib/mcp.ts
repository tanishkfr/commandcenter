...
export function createMcpServer(){
  ...
  server.tool('reviewMemoryArtifact','Accept a captured memory into project context or reject it from the review inbox',{artifactId:z.string(),action:z.enum(['accept','reject'])},async({artifactId,action})=>textResult(await creativeMemoryStore.reviewArtifact(artifactId,action)));
