...
export function createMcpServer(){
  ...
  server.tool('createCreativeProject','Create a new personal creative-memory project',{name:z.string().min(1),description:z.string().optional(),color:z.string().regex(/^#[0-9a-f]{6}$/i,'Use a six-digit hex color.').optional()},async args=>textResult(await creativeMemoryStore.createProject(args)));
