...
const steps=[
  ...
  {label:'Your studio',icon:Database},
  {label:'AI',icon:Brain},
  {label:'MCP',icon:Plug},
...
export default function Onboarding({data,onClose,onConfigured}:Props){
  ...
  return <div className="onboarding-backdrop">
    <section ref={shellRef} className="onboarding-shell" role="dialog" aria-modal="true" aria-label="Set up Creative Memory Studio">
      ...
      <main className="onboarding-main">
        ...
        <footer className="onboarding-actions">
          ...
          <div>
            {step===0&&<button type="button" className="onboarding-primary" onClick={next}>Enter your studio<ArrowRight size={14}/></button>}
...
function Welcome(){
  return <div className="onboarding-copy welcome-copy"><p className="eyebrow">Welcome to your design studio</p><h1>Creative work<br/>should remember itself.</h1><p className="onboarding-lede">This is not a task manager. It is a quiet place to think with AI and let every useful conversation become durable project knowledge.</p>
    <div className="memory-loop"><div><span><MessageCircleMore size={18}/></span><strong>Have a conversation</strong><small>Think naturally. No filing system required.</small></div><ArrowRight size={16}/><div><span><Sparkles size={18}/></span><strong>Capture the session</strong><small>Studio finds decisions, ideas, and questions.</small></div><ArrowRight size={16}/><div><span><Brain size={18}/></span><strong>Build project memory</strong><small>Search what changed, when, and why.</small></div></div>
  </div>
}
...
function AiSetup({status,apiKey,model,busy,onKey,onModel,onConnect,onSkip}:{status:ConnectionStatus|null;apiKey:string;model:string;busy:boolean;onKey:(value:string)=>void;onModel:(value:string)=>void;onConnect:()=>void;onSkip:()=>void}){
  return <div className="onboarding-copy"><p className="eyebrow">AI connection</p><h1>Choose how Studio<br/>thinks with you.</h1><p className="onboarding-lede">NVIDIA NIM gives you richer project-aware conversation and more nuanced memory extraction. Local intelligence remains available without a key.</p>
    {status?.runtime==='vercel'&&!status.aiConfigured?<div className="cloud-setup-card"><ShieldCheck size={17}/><div><strong>Configure AI in Vercel</strong><p>Add <code>NVIDIA_API_KEY</code> and <code>NVIDIA_MODEL</code> under Project Settings → Environment Variables, then redeploy. Studio remains usable without them.</p></div></div>:status?.aiConfigured?<div className="connection-success"><span><Check size={15}/></span><div><strong>NVIDIA NIM is connected</strong><p>{status.aiModel} is ready for conversation and capture.</p></div></div>:<div className="connection-form">
      ...
      <div className="connection-choice"><button type="button" className="onboarding-primary" onClick={onConnect} disabled={busy||apiKey.length<20}>{busy?<Loader2 className="spin" size={14}/>:<Sparkles size={14}/>}Test and connect</button><button type="button" className="quiet-link" onClick={onSkip}>Use local intelligence for now</button></div>
...
function Ready({status,projectName}:{status:ConnectionStatus|null;projectName:string}){
  ...
  return <div className="onboarding-copy ready-copy"><p className="eyebrow">Setup complete</p><h1>Your studio is ready<br/>to remember.</h1><p className="onboarding-lede">Start with something unresolved: an observation, a design tension, a question, or a direction you are unsure about.</p>
    ...
    <div className="first-prompt"><Sparkles size={16}/><div><small>Try this first</small><p>"I am deciding between two directions. Help me understand the tradeoff without rushing to an answer."</p></div></div>
  </div>
}
